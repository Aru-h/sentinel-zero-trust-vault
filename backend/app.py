...
RESTRICTED_ATTEMPTS: dict[str, dict] = {}
...
def _track_restricted_attempt(session_id: str, classification: str) -> bool:
    if classification not in ('Restricted', 'Confidential'):
        return False

    now = time.time()
    attempt = RESTRICTED_ATTEMPTS.get(session_id)
    if (not attempt) or (now - attempt['first_attempt_time'] > 60):
        attempt = {
            'count': 0,
    ...
    attempt['count'] += 1
    if attempt['count'] > 5 and not attempt['termination_pending']:
        attempt['termination_pending'] = True
        attempt['termination_deadline'] = now + 10

    RESTRICTED_ATTEMPTS[session_id] = attempt
    return attempt['termination_pending']


def _enforce_pending_termination():
    ...
    tracked = RESTRICTED_ATTEMPTS.get(current_session_id)
    if not tracked or not tracked.get('termination_pending'):
        return

    now = time.time()
    if now >= tracked['termination_deadline']:
        user_id = session.get('user_id')
        user_name = session.get('user_name', 'Unknown')
        user_role = session.get('user_role', 'Unknown')
        if user_id:
            ACTIVE_SESSIONS.pop(user_id, None)
        log_access_event(user_id or 'unknown', user_name, user_role, '-', 'Session Security Control', 'SESSION_TERMINATED', 'excessive_restricted_attempts')
        session.clear()
        RESTRICTED_ATTEMPTS.pop(current_session_id, None)
        return jsonify({'error': 'Session terminated due to excessive restricted access attempts'}), 401

    remaining_seconds = max(1, int(tracked['termination_deadline'] - now + 0.999))
    return jsonify({
        'access': 'TERMINATION_PENDING',
        'countdown': remaining_seconds,
        'message': 'Your current session will be terminated due to excessive restricted access attempts.'
    }), 200
...
app.before_request(_enforce_pending_termination)
...
def extract_identity():
    ...
    if 'user_id' in session:
        ...
        if not expected_session_id or expected_session_id != session.get('session_id'):
            stale_session_id = session.get('session_id')
            if stale_session_id:
                RESTRICTED_ATTEMPTS.pop(stale_session_id, None)
            session.clear()
            g.user_id = None
            return
...
def logout():
    ...
    if session.get('user_id'):
        ACTIVE_SESSIONS.pop(session['user_id'], None)
    if session_id:
        RESTRICTED_ATTEMPTS.pop(session_id, None)
    session.clear()
...
def access_document():
    ...
    session_id = session.get('session_id')
    if session_id and _track_restricted_attempt(session_id, doc_meta['classification']):
        return jsonify({
            'access': 'TERMINATION_PENDING',
            'countdown': 10,
            'message': 'Your current session will be terminated due to excessive restricted access attempts.'
